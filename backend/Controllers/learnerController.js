...
export const myLearning = async (req, res) => {
  try {
    const userId = req.user.user_id || req.user.id || req.user.userId; // student id from token

    const [myLearning] = await db.query(`
      SELECT 
        c.course_id,
        c.instructor_id,
        c.title,
    ...
    return res.status(200).json({
      success: true,
      message: "My Learning Courses Retrieved",
      courses: myLearning, // ✅ return data
    });
